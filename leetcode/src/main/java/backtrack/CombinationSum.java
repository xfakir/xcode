...
public class CombinationSum {
    ...
    public List<List<Integer>> solution2(int[] candidates,int target) {
        ...
        backtrack2(candidates,target,result,new ArrayList<>(),0);
        return result;
    }

    public void backtrack2(int[] candidates,int target,List<List<Integer>> result,List<Integer> list,int begin) {
        if (target == 0) {
            result.add(new ArrayList<>(list));
            return;
        }
        for (int i = begin; i < candidates.length; i++) {
            ...
            list.add(candidates[i]);
            backtrack2(candidates,target-candidates[i],result,list,i);
            list.remove(list.size()-1);
        }
    }
}
